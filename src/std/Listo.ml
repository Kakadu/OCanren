...
type ('a, 'l) list = Nil | Cons of 'a * 'l [@@deriving gt ~options:{show; gmap; eq; compare; foldl; foldr; fmt}]
...
module X =
  struct
    type ('a,'b) t = ('a, 'b) list [@@deriving gt ~options:{show; gmap; eq; compare; foldl; foldr; fmt}]
