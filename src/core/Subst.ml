...
let of_list =
  ListLabels.fold_left
    ...
    ~f:
      (let open Binding in
      fun subst { var; term } ->
        if not @@ Term.VarMap.mem var subst
        then Term.VarMap.add var term subst
        else invalid_arg "OCanren fatal (Subst.of_list): invalid substituion")
;;
...
let unify ?(subsume = false) ?(scope = Term.Var.non_local_scope) env subst x y =
  ...
  with
  | Term.Different_shape _ | Unification_failed | Occurs_check -> None
;;
