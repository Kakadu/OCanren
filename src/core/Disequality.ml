...
module Answer = struct
  ...
  let add env t var term =
    ...
    with
    | Not_found -> Term.VarMap.add var (S.singleton term) t
  ;;

  let mem env t var term =
    try S.mem term @@ Term.VarMap.find var t with
    | Not_found -> false
  ;;

  let extract t v =
    try S.elements @@ Term.VarMap.find v t with
    | Not_found -> []
  ;;
...
module Disjunct : sig
  ...
  let pp ppf t =
    ...
    Term.VarMap.iter
      (fun k v ->
        ...
        Format.fprintf ppf "@[%d -> %s@] " idx (Term.show @@ Obj.repr v))
      t;
    Format.fprintf ppf ")@]"
  ;;
  ...
  let rec recheck env subst t =
    ...
    | Violated ->
      if Term.VarMap.is_empty unchecked
  ...
  let has_wildcard_in_t mapa = Term.VarMap.exists (fun _k -> has_wilcard_inside) mapa
  ...
  let subsumed env subst t1 t2 =
    ...
    let ans =
      ...
      then false
      else Subst.(subsumed env (of_map t2) (of_map t1))
    in
...
module Conjunct : sig
  ...
  let recheck env subst t =
    ...
    M.fold
      (fun id disj acc ->
        try M.add id (Disjunct.recheck env subst disj) acc with
        | Disequality_fulfilled -> acc)
      t
      M.empty
  ;;
  ...
  let diff env subst t' t =
    M.fold
      (fun id disj' (refined, added) ->
        ...
        with
        | Not_found -> refined, M.add id disj' added)
      t'
      (M.empty, M.empty)
  ;;
...
let pp ppf t =
  Format.fprintf ppf "{| ";
  Term.VarMap.iter
    (fun k v -> Format.fprintf ppf " %d -> %a, " Term.VarTbl.(k.index) Conjunct.pp v)
    t;
  Format.fprintf ppf "|}"
;;
...
let add env subst cstore x y extra =
...
let recheck env subst cstore bs extra =
...
let%expect_test _ =
  ...
  [%expect {xxx| {| |} |xxx}];
  let (Some (store1, ())) =
    add env Subst.empty store0 q (Obj.magic (__, Obj.magic 1)) ()
  in
  Format.printf "%a\n%!" pp store1;
  [%expect {xxx|
  ...
  |xxx}];
  let (Some (store2, ())) =
    add env Subst.empty store1 q (Obj.magic (Obj.magic 1, __)) ()
  in
  [%expect {xxx| |xxx}];
