...
module Answer = struct
  ...
  let add env t var term =
    ...
    with Not_found -> Term.VarMap.add var (S.singleton term) t

  let mem env t var term =
    try S.mem term @@ Term.VarMap.find var t with Not_found -> false
    ...
    Term.VarMap.for_all
      (fun var terms' ->
        try
          ...
          S.for_all
            (fun term' ->
              S.exists (fun term -> Subst.Answer.subsumed env term term') terms)
            terms'
...
module Disjunct : sig
      ...
      ~f:
        (let open Subst.Binding in
         fun acc { var; term } ->
           ...
           else Term.VarMap.add var term acc)

  let of_list = update Term.VarMap.empty
  let samplevar t = fst @@ Term.VarMap.max_binding t

  type status = Fulfiled | Violated | Refined of Subst.Binding.t list
  ...
  let rec recheck env subst t =
    ...
    | Violated ->
        if Term.VarMap.is_empty unchecked then raise Disequality_violated
        else recheck env subst unchecked
  ...
  let reify env subst t =
    Term.VarMap.fold
      (fun var term xs -> Subst.(Binding.{ var; term } :: xs))
      t []
  ...
  let is_relevant env subst t fv =
    ...
    Term.VarMap.for_all
      (fun var term ->
        ...
        | Some u -> Term.VarSet.mem u fv
        | None -> false)
      t

  let freevars env subst t =
    Term.VarMap.fold
      (fun _ term acc -> Term.VarSet.union acc @@ Subst.freevars env subst term)
      t Term.VarSet.empty

  let subsumed env subst t t' = Subst.(subsumed env (of_map t') (of_map t))
end
...
module Conjunct : sig
  ...
  let recheck env subst t =
    M.fold
      (fun id disj acc ->
        try M.add id (Disjunct.recheck env subst disj) acc
  ...
  let diff env subst t' t =
    M.fold
      (fun id disj' (refined, added) ->
        ...
        with Not_found -> (refined, M.add id disj' added))
      t' (M.empty, M.empty)

  let remove_subsumed env subst cs =
    M.fold
      (fun id disj acc ->
        if M.exists (fun _ disj' -> Disjunct.subsumed env subst disj' disj) acc
        then
          ...
          let acc =
            M.filter
              (fun _ disj' -> not (Disjunct.subsumed env subst disj disj'))
              acc
          in
          M.add id disj acc)
      cs M.empty
        ...
        M.fold
          (fun id disj (fv', conj) ->
            ...
            if Disjunct.is_relevant env subst disj fv then
              let fv' =
                Term.VarSet.union fv' @@ Disjunct.freevars env subst disj
              in
    ...
    M.fold
      (fun _ disj acc ->
        ...
        ListLabels.map acc ~f:(fun answ ->
             ...
             *)
            if
              List.exists (fun { var; term } -> Answer.mem env answ var term) bs
            then [ answ ]
...
type oldt = Conjunct.t Term.VarMap.t
type t = DC of Conjunct.t Term.VarMap.t [@@unboxed]

let pp ppf (DC map : t) =
  let open Format in
  fprintf ppf "{| ";
  Term.VarMap.iter
    (fun k v -> Format.fprintf ppf "%d -> %a; " k.Term.Var.index Conjunct.pp v)
    map;
  fprintf ppf " |}"

let empty : t = DC Term.VarMap.empty

(* merges all conjuncts (linked to different variables) into one *)
let combine env subst : t -> Conjunct.t =
 fun (DC cstore) ->
  Term.VarMap.fold
    (fun _ -> Conjunct.merge_disjoint env subst)
    cstore Conjunct.empty

let merge_disjoint env subst : t -> t -> t =
 fun (DC a) (DC b) ->
  DC
    (Term.VarMap.union
       (fun _ c1 c2 ->
...
let update env subst conj : t -> t =
 fun cstore -> merge_disjoint env subst (DC (Conjunct.split conj)) cstore

let add env subst (DC cstore : t) x y : t option =
  try Some (update env subst (Conjunct.make env subst x y) (DC cstore)) with
  | Disequality_fulfilled -> Some (DC cstore)
  | Disequality_violated -> None

let rec recheck env subst (DC cstore : t) bs : t option =
  let open Format in
  Format.printf "@[[[[ ";
  List.iter
    (fun { Subst.Binding.var; term } ->
      Format.printf "%d -> %s; " var.Term.Var.index (Term.show term))
    bs;
  log " ]]]@]";

  let helper : Term.Var.t -> oldt -> oldt =
   fun var cstore ->
    (*  *)
    log "%s _.%d" __FUNCTION__ var.Term.Var.index;
    try
      let conj = Term.VarMap.find var cstore in
      log "  conj      = %a" Conjunct.pp conj;
      let cstore = Term.VarMap.remove var cstore in
      let rechecked = Conjunct.recheck env subst conj in
      log "  rechecked = %a" Conjunct.pp rechecked;
      let (DC updated) = update env subst rechecked (DC cstore) in
      updated
      (* recheck env subst updated bs *)
    with Not_found ->
...
let project env subst dc fv =
  DC Conjunct.(split @@ project env subst (combine env subst dc) fv)
