...
let var x = if is_var x then Some (Obj.magic x) else None

let rec map ~fvar ~fval x =
  let tx = Obj.tag x in
  if (is_box tx) then
    let sx = Obj.size x in
    if is_var1 tx sx x then
      fvar @@ Obj.magic x
    else
      let y = Obj.dup x in
      for i = 0 to sx - 1 do
...
let rec fold2 ~fvar ~fval ~fk ~init x y =
  ...
  | true, true -> begin
    ...
    match is_var1 tx sx x, is_var1 ty sy y with
  ...
  | true, false ->
    is_valid_tag_exn ty;
    let sx = Obj.size x in
    if is_var1 tx sx x then fk init L (Obj.magic x) y else raise (Different_shape (tx, ty))
  | false, true ->
    is_valid_tag_exn tx;
    let sy = Obj.size y in
    if is_var1 ty sy y then fk init R (Obj.magic y) x else raise (Different_shape (tx, ty))
  | false, false ->
    is_valid_tag_exn tx;
    is_valid_tag_exn ty;
    if tx = ty then
