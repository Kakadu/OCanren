...
module TreeLogger = struct
  type node =
    { event : Listener.event
    ; children : StateId.t list
    }
  ...
  let color_of_event event cs =
    Listener.(
      ...
      | Conj ->
        if cs = []
        then Yellow
        else if List.for_all (( == ) Green) cs
        then Green
        else if List.exists (( == ) Red) cs
        then Red
        else if List.exists (( == ) Yellow) cs
        then Yellow
        else White
      | Disj ->
        if cs = []
        then Yellow
        else if List.for_all (( == ) Red) cs
        then Red
        else if List.exists (( == ) Green) cs
        then Green
        else if List.exists (( == ) Yellow) cs
        then Yellow
        else White
  ...
  type box =
    { (* id of the node in the search tree *)
      id : StateId.t (* string that describes the node *)
    ; str : string (* color of node *)
    ; c : color (* list of children nodes *)
    ; inner : box list
          (* flag that specifies whether the id of node should be printed along with text *)
    ; with_id : bool (* optional link to another location in the search tree *)
  ...
  let rec fold_boxes = function
    | [] -> []
    | [ b ] -> [ b ]
    | ({ id = id1 } as b1) :: { id = id2; inner } :: bs
      when id1 = id2 && List.length inner = 1 ->
      let [ b2 ] = inner in
      b1 :: (fold_boxes @@ (b2 :: bs))
    | bs -> bs
  ;;
  ...
  let list_max (x :: xs) = List.fold_left max x xs
  ...
  let make_pinfo filter show_unif (id, event) children =
    (* Format.printf "make_pinfo id=%s\n%!" (StateId.show id); *)
    Listener.(
      let boxes =
        fold_continue_box (List.concat @@ List.map (fun { boxes } -> boxes) children)
      in
      ...
      match event with
      | Conj when boxes = [] -> { c; boxes = []; answers }
      | Disj when boxes = [] -> { c; boxes = []; answers }
      | Conj ->
        let boxes = fold_boxes boxes in
        if List.length boxes = 1
        then { c; boxes; answers }
        else if List.for_all is_leaf boxes
        then (
          let ids = List.map (fun { id } -> id) boxes in
          let id = list_max ids in
          let str =
            "(" ^ (String.concat ") &&& (" @@ List.map (fun { str } -> str) boxes) ^ ")"
          in
      ...
      | Cont id ->
        ...
        then (
          let box =
            make_box id "continue" c ~inner:boxes ~with_id:false ~link:(Some id)
          in
          { c; boxes = [ box ]; answers })
        else { c; boxes = []; answers }
      | Unif _ ->
        if show_unif
        then (
  ...
  let make_labels : box list -> int H.t =
   fun boxes ->
 ...
 ;;
  ...
  let print_tree ff color labels roots =
    ...
    let rec helper { id; c; str; inner; with_id; link; answers } =
      ...
      if answers <> [] then print_answers c answers else ();
  ...
  let create () =
    object (self)
      ...
      method on_event e pid id =
        let _ = H.add tbl id @@ make_node e in
