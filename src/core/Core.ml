...
let (===) x y st =
  let module T = struct
    [%%if defined stats]
    let t = unification_incr (); Timer.make ()
    [%%endif]
  end in

  let ans = State.unify x y st in
  let module _  = struct
    [%%if defined stats] let () = unification_time_incr T.t [%%endif]
  end in
  match ans with
  | Some st -> success st
  | None    -> failure st
...
let conj f g st =
  let module _  = struct
    [%%if defined stats] let () = conj_counter_incr () [%%endif]
  end in
  Stream.bind (f st) g
...
let disj f g st =
  ...
  let module _  = struct
    [%%if defined stats] let () = disj_counter_incr () [%%endif]
  end in
