      ...
      let rec helper x = Term.map x ~fval:Term.repr
        ~fvar:begin fun v -> Term.repr @@
          ...
          with Not_found ->
            ...
            let ctr = Stdlib.List.map helper v.Term.Var.constraints |> Stdlib.List.sort Term.compare in
        ...
        let rec helper forbidden t = Term.map t ~fval:Term.repr
          ~fvar:begin fun v -> Term.repr @@
            if Term.VarSet.mem v forbidden then v
                ...
                |> Stdlib.List.filter begin fun dt ->
                  match Env.var env dt with
                  | Some u -> not @@ Term.VarSet.mem u forbidden
                  | None   -> true
                end
                |> Stdlib.List.map (fun x -> helper (Term.VarSet.add v forbidden) x)
...
let debug_var v reifier call = fun st ->
  let xs = Stdlib.List.map (fun answ ->
