...
let ( === ) x y st =
  ...
  in
  let ans = State.unify x y st in
  let module _ = struct
    [%%if defined stats]

    let () = unification_time_incr T.t
...
let ( =/= ) x y st =
  ...
  | Some st ->
    (* let module _  = struct
           [%%if defined stats] let () = delay_counter_incr () [%%endif]
         end in *)
    success st
  | None -> failure st
;;
...
let structural : 'a -> ('a, 'b) Reifier.t -> ('b -> bool) -> goal =
 fun term rr k st ->
...
let ( ?& ) gs st =
  match gs with
  | [ h ] -> h st
  | h :: tl -> List.fold_left (fun acc x -> Stream.bind acc x) (h st) tl
  | [] -> assert false
;;
...
module NUMERAL_TYPS = struct
  type ('a, 'c, 'e, 'f, 'g) one =
    unit
    -> (('a ilogic -> goal) -> State.t -> 'a ilogic * State.t Stream.t)
       * ('c ilogic -> Env.t -> 'c reified)
       * ('e -> 'e)
       * (('f -> 'g) -> 'f -> 'g)
  ...
  type ('a, 'b, 'c, 'd, 'e, 'f, 'g, 'h, 'i, 'j, 'k, 'l, 'm, 'n, 'o, 'p, 'q, 'r) four =
    unit
    -> (('a ilogic -> 'b ilogic -> 'c ilogic -> 'd ilogic -> goal)
        -> State.t
        -> 'a ilogic * ('b ilogic * ('c ilogic * ('d ilogic * State.t Stream.t))))
       * ('e ilogic * ('f ilogic * ('g ilogic * 'h ilogic))
          -> Env.t
          -> 'e reified * ('f reified * ('g reified * 'h reified)))
       * ('i * ('j * ('k * ('l * 'm))) -> ('i * ('j * ('k * 'l))) * 'm)
       * (('n -> 'o -> 'p -> 'q -> 'r) -> 'n * ('o * ('p * 'q)) -> 'r)
end
...
let one : (_, _, _, _, _) NUMERAL_TYPS.one =
 fun () -> LogicAdder.(succ zero), ReifyTuple.one, ExtractDeepest.ext2, Uncurry.one
;;
...
module Tabling = struct
  let succ n () =
    ...
    let sc =
      (Curry.succ : (('a -> 'b) -> 'c) -> ((_ ilogic as 'k) * 'a -> 'b) -> 'k -> 'c)
    in
