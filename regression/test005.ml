...
let show_env_logic = show(List.logic) @@ show(logic) (show MiniKanrenStd.Pair.t (show(logic) (fun s -> s)) show_llam)

let pair c p = ManualReifiers.pair ManualReifiers.string glam_reifier c p

let env_reifier c xs = List.reify pair c xs

let show_env  = show(List.ground) @@ GT.(show MiniKanrenStd.Pair.t) show_string  GLam.show_rlam
let show_envl = show(List.logic ) @@ show(logic) GT.(show MiniKanrenStd.Pair.t show_stringl GLam.show_llam)
