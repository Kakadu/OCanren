...
let is_fresh =
  classify_name ~f:(function
      | Lident "fresh" | Lident "fresh_" -> true
      | _ -> false)
;;
...
let classify_fresh e =
  match e.pexp_desc with
  | Pexp_ident { txt = Lident "fresh" } -> `NormalFresh
  | Pexp_ident { txt = Lident "fresh_" } -> `NamedFresh
  | _ -> `Other
;;
